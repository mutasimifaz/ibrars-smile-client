...
const Booking = ({ service }) => {
    ...
    const onSubmit = data => {
        fetch('https://ibrars-smile.herokuapp.com/services/booking', {
            method: 'POST',
            headers: {
        ...
        <div>
            <div className="mb-5 mt-5">
                <form className="flex flex-col w-80 mx-auto" onSubmit={handleSubmit(onSubmit)}>
                    ...
                    <input defaultValue={user?.email} className="border-2 border-gray-300 m-2 px-3 hover:border-red-600 py-2 rounded " {...register("email", { required: true })} placeholder="Email" />
                    ...
                    <button onClick={handleClick} type="submit" className="button-green">Buy Now <i className="fa-solid fa-cart-shopping ico"></i></button>
                </form>
