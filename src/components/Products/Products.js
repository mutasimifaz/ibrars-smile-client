...
const Products = () => {
  ...
  useEffect(() => {
    fetch("https://ibrars-smile.herokuapp.com/products")
  ...
  const handleDeleteProduct = (id) => {
    ...
    fetch(`https://ibrars-smile.herokuapp.com/deleteProduct/${id}`, {
      method: "DELETE",
  ...
  const handleAddToCart = (index) => {
    ...
    fetch(`https://ibrars-smile.herokuapp.com/addOrders`, {
      method: "POST",
      headers: { "content-type": "application/json" },
        ...
        <div className="row container text-center">
          {products.map((pd, index) => (
            <div className="col-md-4">
              <div className="product border p-2 m-2">
                <h5>{pd?.name}</h5>
                <h5>{pd?.price}</h5>
